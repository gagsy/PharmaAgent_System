...
with t2:
    ...
    if os.path.exists(log_path):
        import pandas as pd
        # Load the actual data trail generated by the agents
        try:
            df = pd.read_csv(log_path)
            
            # Professional KPI Summary for the Auditor
            st.info(f"Showing last {len(df)} transactions for legal compliance.")
            
            # Display the data in a searchable, high-contrast table
            st.dataframe(
                df.sort_index(ascending=False), # Show newest scans first
                use_container_width=True,
                hide_index=True
            )
            
            # Export feature for Hospital Administration
            st.download_button(
        ...
        # Professional fallback if no scans have been performed yet
